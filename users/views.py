...
def signup_view(request):
    ...
    if not username or not email or not password or not name or not public_key:
        return Response({"error": "All fields (username, email, password, public_key) are required."}, status=status.HTTP_400_BAD_REQUEST)
